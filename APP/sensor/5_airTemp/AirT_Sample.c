...
float data_sample_buf_5[5 * SAMPLE_DATA_NUM];//5min采样数据保存缓冲区
float data_sample_buf_history_5[5 * SAMPLE_DATA_NUM];//5min采样数据保存缓冲区
unsigned char sample_qc_5[MAX_SENSOR_NUM][19];      //5min第二个值 和采样值有关
...
unsigned char Element_MinSample(sensors_data_t *sensors_tempdata)
{
  ...
  Self_test();
	...
	if(Num_sample_5 <= 120)
	{
	    memset(&data_sample_buf_5[Num_sample_5],0,sizeof(float) * 30);	//采样数据缓冲区清0
...
void SampleData_Init(sensors_data_t *sensors_tempdata)
{
	...
	memset(sample_qc_history,0,sizeof(sample_qc_history));//采样标志位缓冲区清0
	
    memset(data_sample_buf_5,0,sizeof(data_sample_buf_5));//采样数据缓冲区清0
    memset(data_sample_buf_history_5,0,sizeof(data_sample_buf_history_5));//采样数据缓冲区清0
    memset(sample_qc_5,0,sizeof(sample_qc_5));//采样标志位缓冲区清0
	memset(sample_qc_history_5,0,sizeof(sample_qc_history_5));//采样标志位缓冲区清0
