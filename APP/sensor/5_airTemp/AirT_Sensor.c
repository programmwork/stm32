...
void Init_sys_cfg()
{
    ...
    //气温
    bcm_info.sensor.qm[0].min        = -75;          //下限
    bcm_info.sensor.qm[0].max        = 80;           //上限
    bcm_info.sensor.qm[0].doubtful   = 3;            //存疑门限
    bcm_info.sensor.qm[0].err        = 5;            //错误门限
    bcm_info.sensor.qm[0].changerate = 0.1;          //最小应该变化的速率
    //5min 气温
    bcm_info.sensor.qm[1].min        = -75;          //下限
    bcm_info.sensor.qm[1].max        = 80;           //上限
    bcm_info.sensor.qm[1].doubtful   = 3;            //存疑门限
    bcm_info.sensor.qm[1].err        = 5;            //错误门限
    bcm_info.sensor.qm[1].changerate = 0.1;          //最小应该变化的速率
    ...
    bcm_info.sensor.data_num   = 4;                //观测要素数量
...
void check_event_sample(void)
{
    ...
    if(m_tempdata.event.msecevent==true)
    {
        m_tempdata.event.msecevent = false;
