...
void Init_sys_cfg()
{
    ...
    bcm_info.sensor.qm[0].err        = 15;         //错误门限
    bcm_info.sensor.qm[0].changerate = 1;          //最小应该变化的速率
  
    //设置或读取各要素采样值质量控制参数(QS)
    //湿度
    bcm_info.sensor.qs[0].min    = 0;       //下限
    bcm_info.sensor.qs[0].max    = 100;     //上限
    bcm_info.sensor.qs[0].change = 5;       //允许最大变化值
	...
	bcm_info.sensor.ce = 2;               // 气压传感器类型  1-华云210         2-baro 1
    bcm_info.sensor.ce_H = 1;             // 湿度传感器类型  1-罗卓尼克           2-155
