...
unsigned char Element_SecSample(sensors_data_t *sensors_tempdata)
{
  ...
  {
    temp_sample_event= SoilMoisture_engine(temp_value);//读计数清零计数
...
//---------------------------head  finish--------------------------
    ...
    //---------------------------ARG10---------------------------
    strcpy(Header_packet+lenlen,"ARG10");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    //mindata
    ...
    //---------------------------ARG20--------------------------
    strcpy(Header_packet+lenlen,"ARG20");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    //mindata
    Pack_Data(&Mindata_temp.sensor[1].mindata, &lenlen, Header_packet, 3);
    
    //---------------------------ARG30---------------------------
    strcpy(Header_packet+lenlen,"ARG30");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    //mindata
    Pack_Data(&Mindata_temp.sensor[2].mindata, &lenlen, Header_packet, 3);

    
    //---------------------------ARG40---------------------------
    strcpy(Header_packet+lenlen,"ARG40");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    //mindata
    Pack_Data(&Mindata_temp.sensor[3].mindata, &lenlen, Header_packet, 3);

    
    //---------------------------ARG50---------------------------
    strcpy(Header_packet+lenlen,"ARG50");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    //mindata
    Pack_Data(&Mindata_temp.sensor[4].mindata, &lenlen, Header_packet, 3);
    
    //=============标准差ARB10===============================================
   /*

    strcpy(Header_packet+lenlen,"ARB10");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    ...
    //======================标准差ARB20=======================================================
    strcpy(Header_packet+lenlen,"ARB20");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    ...
    //====================标准差ARB30=========================================================
    strcpy(Header_packet+lenlen,"ARB30");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    ...
    if(num_mins == 1)//需要存储分钟数据，来自主动调用
    { //数据保存至flash结构体
      standardDeviation_1 = StandardDeviation(data_sample_buf_history+SAMPLE_COUNT*2, sample_qc_history[2], SAMPLE_COUNT);
      if(INVALID_DATA != Mindata_temp.sensor[2].mindata.data)
          Mindata_temp.Standard_Deviation[2] = standardDeviation_1 * 100;
      else
          Mindata_temp.Standard_Deviation[2] = INVALID_DATA;

      FLASH_Store_MinData.Standard_Deviation[2] = Mindata_temp.Standard_Deviation[2];
    }
    else if((num_mins == 0)||(num_mins == 2))//不需要存储的分钟数据，来自历史数据读取调用
    {
        if(INVALID_DATA != Mindata_temp.sensor[2].mindata.data)
            Mindata_temp.Standard_Deviation[2] = FLASH_Read_MinData.Standard_Deviation[2];
        else
            Mindata_temp.Standard_Deviation[2] = INVALID_DATA;
    }
    else ;

    Pack_Datal(&Mindata_temp.Standard_Deviation[2], &lenlen, Header_packet, 4);
    
    //======================标准差ARB40=======================================================
    strcpy(Header_packet+lenlen,"ARB40");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    ...
    if(num_mins == 1)//需要存储分钟数据，来自主动调用
    { //数据保存至flash结构体
      standardDeviation_1 = StandardDeviation(data_sample_buf_history+SAMPLE_COUNT*3, sample_qc_history[3], SAMPLE_COUNT);
      if(INVALID_DATA != Mindata_temp.sensor[3].mindata.data)
          Mindata_temp.Standard_Deviation[3] = standardDeviation_1 * 100;
      else
          Mindata_temp.Standard_Deviation[3] = INVALID_DATA;
      
      FLASH_Store_MinData.Standard_Deviation[3] = Mindata_temp.Standard_Deviation[3];
    }
    else if((num_mins == 0)||(num_mins == 2))//不需要存储的分钟数据，来自历史数据读取调用
    {
        if(INVALID_DATA != Mindata_temp.sensor[3].mindata.data)
            Mindata_temp.Standard_Deviation[3] = FLASH_Read_MinData.Standard_Deviation[3];
        else
            Mindata_temp.Standard_Deviation[3] = INVALID_DATA;
    }
    else ;

    Pack_Datal(&Mindata_temp.Standard_Deviation[3], &lenlen, Header_packet, 4);
    
    //=========================标准差ARB50====================================================
    strcpy(Header_packet+lenlen,"ARB50");
    lenlen=strlen((char *)Header_packet);
    Header_packet[lenlen]=',';
    lenlen++;
    ...
    if(num_mins == 1)//需要存储分钟数据，来自主动调用
    { //数据保存至flash结构体
      standardDeviation_1 = StandardDeviation(data_sample_buf_history+SAMPLE_COUNT*4, sample_qc_history[4], SAMPLE_COUNT);
      if(INVALID_DATA != Mindata_temp.sensor[4].mindata.data)
          Mindata_temp.Standard_Deviation[4] = standardDeviation_1 * 100;
      else
          Mindata_temp.Standard_Deviation[4] = INVALID_DATA;
      
      FLASH_Store_MinData.Standard_Deviation[4] = Mindata_temp.Standard_Deviation[4];
    }
    else if((num_mins == 0)||(num_mins == 2))//不需要存储的分钟数据，来自历史数据读取调用
    {
        if(INVALID_DATA != Mindata_temp.sensor[4].mindata.data)
            Mindata_temp.Standard_Deviation[4] = FLASH_Read_MinData.Standard_Deviation[4];
        else
            Mindata_temp.Standard_Deviation[4] = INVALID_DATA;
    }
    else ;
    
    Pack_Datal(&Mindata_temp.Standard_Deviation[4], &lenlen, Header_packet, 4);
*/

    //memset(data_sample_buf_history,0,sizeof(data_sample_buf_history));
    //ARG10 qc
    ...
    lenlen=strlen((char *)Header_packet);
    //ARG30 qc
    S32ToStrAlignRight(Mindata_temp.sensor[2].mindata.qc, 1, Header_packet+lenlen);
    lenlen=strlen((char *)Header_packet);
    //ARG40 qc
    S32ToStrAlignRight(Mindata_temp.sensor[3].mindata.qc, 1, Header_packet+lenlen);
    lenlen=strlen((char *)Header_packet);
    //ARG50 qc
    S32ToStrAlignRight(Mindata_temp.sensor[4].mindata.qc, 1, Header_packet+lenlen);
    lenlen=strlen((char *)Header_packet);
/*
    //ABI10 qc
