...
int GetMinData(char *Header_packet,sensors_data_t *sensors_tempdata,unsigned char num_mins)
{
  ...
  S32ToStrAlignRight(bcm_info.common.version, 3, Header_packet+3);//版本号
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;

  S32ToStrAlignRight(bcm_info.common.qz, 5, Header_packet+lenlen);//设备区站号
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;

  S32ToStrAlignRight(bcm_info.common.Lat.degree, 2, Header_packet+lenlen);//设备纬度
  lenlen=strlen((char *)Header_packet);
  S32ToStrAlignRight(bcm_info.common.Lat.min, 2, Header_packet+lenlen);//设备纬度
  lenlen=strlen((char *)Header_packet);
  S32ToStrAlignRight(bcm_info.common.Lat.sec, 2, Header_packet+lenlen);//设备纬度
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;

  S32ToStrAlignRight(bcm_info.common.Long.degree, 3, Header_packet+lenlen);//设备经度
  lenlen=strlen((char *)Header_packet);
  S32ToStrAlignRight(bcm_info.common.Long.min, 2, Header_packet+lenlen);//设备经度
  lenlen=strlen((char *)Header_packet);
  S32ToStrAlignRight(bcm_info.common.Long.sec, 2, Header_packet+lenlen);//设备经度
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;
  
  S32ToStrAlignRight(bcm_info.common.High, 5, Header_packet+lenlen);//设备海拔
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;

  S32ToStrAlignRight(bcm_info.common.st, 2, Header_packet+lenlen);//设备服务类型
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;
  ...
  bcm_info.common.fi = 1;
  S32ToStrAlignRight(bcm_info.common.fi, 3, Header_packet+lenlen);//帧标识
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;
  ...
  {
  S32ToStrAlignRight(bcm_info.sensor.data_num, 3, Header_packet+lenlen);//观测要素数量
  lenlen=strlen((char *)Header_packet);
  Header_packet[lenlen]=',';
  lenlen++;

  //---------------------------state bcm_information---------------------------
...
//---------------------------head  finish--------------------------
    ...
    for(i = 0;i<sensor_num;i++)
    {
        ...
        if(num_mins == 1)//需要存储分钟数据，来自主动调用
        { //数据保存至flash结构体
          standardDeviation_1 = StandardDeviation(data_sample_buf + SAMPLE_COUNT * i, sample_qc[i], SAMPLE_COUNT);
